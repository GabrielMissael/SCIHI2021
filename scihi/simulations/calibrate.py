...
def Calibrate(PATHS, dates, calibration='Chisq'):
    ...
    data = temp.values
    data_gsm = pd.read_hdf('data/calibration/Tgsm.hdf5')
    ...
    if calibration == 'Chisq':
        ...
        for i in range(len(freqs)):
            K[i] = op.minimize(fun, 1, args=(
                T_meas[i, :], T_gsm[i, :], T_std[i, :])).x[0]
        K = K*1e-19
...
def Check_quality(PATH, dates, savepath=False):
    ...
    if savepath != False:
        plt.savefig(savepath+'data_%s_%s.png' %
                    (Date_0, Date_1), bbox_inches='tight')
    plt.show()
