...
# There should be a file named eta_nu.dat in the directory with the efficiency of the antenna.
eta_nu = eta(Freqs)
...
def Filter(PATH, Output_folder='.', outcome=0.):
    ...
    for subdirs, dirs, files in os.walk(PATH):
        # Inore hidden folders (ipynb checkpoints for example)
        ...
        for file in files:
            ...
            if data.size == 0:
                ...
                Tamb = round(np.genfromtxt(path, comments='!',
                             skip_header=18, max_rows=1)[1]+273.15, 2)
                # Johnson-noise calibration coefficient
                Kjnc = Tamb/(T_50ohm-T_short)
                antenna.append(T_antenna), measure.append(
                    T_measure), K_jnc.append(Kjnc)
        ...
        if i >= 0 and i < len(folders) and short and antenna and _50ohm and measure and K_jnc:
            ...
            short_table = pd.DataFrame(
                short[mask], index=Freqs[mask], columns=short_date)
            short_table.to_hdf(Output_folder+'/short/'+name+'.hdf5', 'df')
            _50ohm_table = pd.DataFrame(
                _50ohm[mask], index=Freqs[mask], columns=_50ohm_date)
            _50ohm_table.to_hdf(Output_folder+'/50ohm/'+name+'.hdf5', 'df')
            antenna_table = pd.DataFrame(
                antenna[mask], index=Freqs[mask], columns=measure_date)
            antenna_table.to_hdf(Output_folder+'/antenna/'+name+'.hdf5', 'df')
            measure_table = pd.DataFrame(
                measure[mask], index=Freqs[mask], columns=measure_date)
            measure_table.to_hdf(Output_folder+'/Tmeas/'+name+'.hdf5', 'df')
            Kjnc_table = pd.DataFrame(
                K_jnc[mask], index=Freqs[mask], columns=measure_date)
